...
struct MockDataFileReader {
  ...
  using phase_dependent_action_list = tmpl::list<Parallel::PhaseActions<
      typename Metavariables::Phase, Metavariables::Phase::Initialization,
      tmpl::list<importer::detail::InitializeDataFileReader>>>;
};
...
struct Metavariables {
  ...
  using const_global_cache_tags =
      tmpl::list<importer::Tags::DataFileName<TestImporter>,
                 importer::Tags::VolumeDataSubgroup<TestImporter>,
                 importer::Tags::ObservationValue<TestImporter>>;
...
SPECTRE_TEST_CASE("Unit.IO.DataImporter.DataFileReaderActions", "[Unit][IO]") {
  ...
  ActionTesting::MockRuntimeSystem<Metavariables> runner{
      {"TestImportVolumeFile.h5", "element_data", 0.}};
  ...
  runner.algorithms<reader_component>()
      ...
      .template threaded_action<importer::ThreadedActions::ReadElementData<
          TestImporter, import_tags_list, TestCallback, element_array>>();
